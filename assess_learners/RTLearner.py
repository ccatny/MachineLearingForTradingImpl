...
class RTLearner(object):
    ...
    def select_factor(self, x_data):
        factor = np.random.randint(low=0, high=x_data.shape[1])
        count = 0
        while np.std(x_data[:, factor]) == 0 and count < x_data.shape[1]:
            factor = np.random.randint(low=0, high=x_data.shape[1])
            count = count + 1
        return factor
    ...
    def build_tree(self, data):
        if data.shape[0] <= self.leaf_size or np.all(
                data[:, -1] == data[0, -1]):
            sub_tree = np.array([['leaf', np.mean(data[:, -1]), np.nan, np.nan]])
            return sub_tree
        else:
            factor = self.select_factor(data[:, 0:-1])
