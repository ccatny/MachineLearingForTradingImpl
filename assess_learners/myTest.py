...
def insample_MAE_result(learner, train_x, train_y, verbose=False):
    ...
    # mae = float(abs(test_y - pred_y).sum() / test_y.shape[0])
    if verbose == True:
        print()
        print("In sample results")
        print(f"MAE: {mae}")
...
if __name__ == "__main__":
    # if len(sys.argv) != 1:
    #   sys.exit(1)
    x, y = get_data(r'C:\Users\chengk\PycharmProjects\ML4T\assess_learners\Data\Istanbul.csv')
    verbose = False
    print(x.shape)
    print(y.shape)
    np.random.seed(903614675)
    ...
    fig, ax = plt.subplots()
    ax.set(title="RMSE for In/Out-Sample Data of DTLearner under different leaf size"
           , xlabel='leaf size'
           , ylabel='RMSE')
    ...
    fig, ax = plt.subplots()
    ax.set(title="RMSE for In/Out-Sample Data of BagLearner under different leaf size"
           , xlabel='leaf size'
           , ylabel='RMSE'
           , ylim=[0.0, 0.01])
    ax.plot(in_sample_result)
    ...
    fig, ax = plt.subplots()
    ax.set(title="MAE for In/Out-Sample Data of DTLearner and LTLearner"
           , xlabel='leaf size'
           , ylabel='MAE'
           , ylim=[0.0, 0.01])
    ...
    fig, ax = plt.subplots()
    ax.set(title="trainingTime of DTLearner and LTLearner"
           , xlabel='leaf size'
